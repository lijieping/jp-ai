...
from app.dao.message_dao import MsgDAO, MsgRole
...
def message_create(msg_create: MsgCreate):
    ...
    with DbSession() as db:
        MsgDAO.add(conv_id=conv_id, role= MsgRole.USER, content= msg_create.content, db = db)
        MsgDAO.add(conv_id=conv_id, role= MsgRole.AI, content= json.dumps([step.to_dict() for step in step_chain]), db = db)
        db.commit()
