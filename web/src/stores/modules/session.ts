...
export const useSessionStore = defineStore('session', () => {
  ...
  // 生成会话标题相关状态和方法
  const shouldGenerateTitle = ref(false);

  // 触发生成标题
  const triggerGenerateTitle = () => {
    shouldGenerateTitle.value = true;
  };

  // 重置生成标题状态
  const resetGenerateTitle = () => {
    shouldGenerateTitle.value = false;
  };
